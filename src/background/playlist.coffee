...
class Playlist
	...
	addVideo: (tab, callback) ->
		...
		if (video = @getVideoByTabId tab.id)?
			...
			return @updateVideo video, tab.title, tab.url, callback
		else
	...
	###
	updateVideo: (video, title, videoUrl, callback) ->
